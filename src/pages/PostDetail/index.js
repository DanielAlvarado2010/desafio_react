...
export default function PostDetail() {
  ...
  const dateTimer = (datePublication) => {
    ...
    const days = Math.floor(
      (currentDay - datePublication.miliseconds) / (1000 * 60 * 60 * 24)
    );
    const hours = Math.floor(
      (currentDay -
        datePublication.miliseconds -
        days * (1000 * 60 * 60 * 24)) /
        (1000 * 60 * 60)
    );
    ...
    <Container>
      <Row className="justify-content-center p-0">
        ...
        <Col xxl={7} xl={7} lg={8} md={11} sm={"auto"} className="cardPost">
          ...
          <div className="card-post">
            <div className="subhead d-flex align-items-center mb-3 flex-wrap">
              ...
              <UserData userName={post?.user} datePost={dates} />
            </div>
            <h1
              className="fs-3xl m:fs-4xl l:fs-5xl fw-bold lh-tight mb-4 long"
              id="title"
            >
              {post?.title}
            </h1>
        ...
        <Col xxl={3} xl={4} lg={4} md={{ span: 11 }}>
          <AsideRightPost avatarImage={post?.userImg} userName={post?.user} />
        </Col>
      </Row>
    </Container>
  );
